...
describe("validateQuery – always-on checks (readOnly=false)", () => {
  ...
  it("throws MULTI_STATEMENT for multiple statements", async () => {
    await expect(validateQuery({ sql: "SELECT 1; DROP TABLE t", readOnly: false })).rejects.toMatchObject({
...
describe("validateQuery – allowlist (readOnly=true)", () => {
  it("accepts a plain SELECT", async () => {
    await expect(validateQuery({ sql: "SELECT 1", readOnly: true })).resolves.toBeUndefined();
  });

  it("accepts EXPLAIN SELECT", async () => {
    await expect(
      validateQuery({ sql: "EXPLAIN SELECT * FROM users", readOnly: true, blockExplain: false }),
  ...
  it("rejects UPDATE", async () => {
    await expect(validateQuery({ sql: "UPDATE t SET x = 1", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_STATEMENT",
    });
  });

  it("rejects DELETE", async () => {
    await expect(validateQuery({ sql: "DELETE FROM t", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_STATEMENT",
    });
  });

  it("rejects CREATE TABLE", async () => {
    await expect(validateQuery({ sql: "CREATE TABLE t (id INT)", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_STATEMENT",
    });
  });

  it("rejects DROP TABLE", async () => {
    await expect(validateQuery({ sql: "DROP TABLE t", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_STATEMENT",
    });
  });

  it("rejects TRUNCATE", async () => {
    await expect(validateQuery({ sql: "TRUNCATE t", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_STATEMENT",
    });
  });
});
...
describe("validateQuery – EXPLAIN inner check (readOnly=true)", () => {
  it("rejects EXPLAIN DELETE", async () => {
    await expect(
      validateQuery({ sql: "EXPLAIN DELETE FROM t", readOnly: true, blockExplain: false }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_EXPLAIN_TARGET" });
  });

  it("rejects EXPLAIN INSERT", async () => {
    await expect(
      validateQuery({ sql: "EXPLAIN INSERT INTO t(x) VALUES (1)", readOnly: true, blockExplain: false }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_EXPLAIN_TARGET" });
  });

  it("rejects EXPLAIN UPDATE", async () => {
    await expect(
      validateQuery({ sql: "EXPLAIN UPDATE t SET x = 1", readOnly: true, blockExplain: false }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_EXPLAIN_TARGET" });
  });
});
...
describe("validateQuery – EXPLAIN ANALYZE (readOnly=true)", () => {
  it("rejects EXPLAIN ANALYZE SELECT", async () => {
    await expect(
      validateQuery({ sql: "EXPLAIN ANALYZE SELECT 1", readOnly: true, blockExplain: false }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_EXPLAIN_ANALYZE" });
  });

  it("rejects EXPLAIN (ANALYZE, BUFFERS) SELECT", async () => {
    await expect(
      validateQuery({ sql: "EXPLAIN (ANALYZE, BUFFERS) SELECT * FROM t", readOnly: true, blockExplain: false }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_EXPLAIN_ANALYZE" });
  });

  it("accepts plain EXPLAIN SELECT", async () => {
    await expect(
      validateQuery({ sql: "EXPLAIN SELECT * FROM t", readOnly: true, blockExplain: false }),
    ).resolves.toBeUndefined();
  });

  it("accepts EXPLAIN (ANALYZE FALSE) SELECT", async () => {
    await expect(
      validateQuery({ sql: "EXPLAIN (ANALYZE FALSE) SELECT * FROM t", readOnly: true, blockExplain: false }),
    ).resolves.toBeUndefined();
  });

  it("accepts EXPLAIN (ANALYZE OFF) SELECT", async () => {
    await expect(
      validateQuery({ sql: "EXPLAIN (ANALYZE OFF) SELECT * FROM t", readOnly: true, blockExplain: false }),
    ).resolves.toBeUndefined();
  });
});
...
describe("validateQuery – deep AST walk (readOnly=true)", () => {
  ...
  it("rejects SELECT FOR UPDATE", async () => {
    await expect(validateQuery({ sql: "SELECT * FROM t FOR UPDATE", readOnly: true })).rejects.toMatchObject(
      {
  ...
  it("rejects SELECT FOR SHARE", async () => {
    await expect(validateQuery({ sql: "SELECT * FROM t FOR SHARE", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_LOCKING",
    });
  });
  ...
  it("accepts a nested subquery SELECT (no mutation)", async () => {
    await expect(
      validateQuery({ sql: "SELECT * FROM (SELECT id FROM t WHERE x > 1) sub", readOnly: true }),
    ).resolves.toBeUndefined();
  });

  it("accepts a read-only CTE", async () => {
    await expect(
      validateQuery({ sql: `WITH cte AS (SELECT id FROM t)SELECT *FROM cte`, readOnly: true }),
    ).resolves.toBeUndefined();
  });
});
...
describe("validateQuery – function denylist (readOnly=true)", () => {
  ...
  it("blocks pg_read_file", async () => {
    await expect(
      validateQuery({ sql: "SELECT pg_read_file('/etc/passwd', 0, 200)", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });

  it("blocks pg_read_binary_file", async () => {
    await expect(
      validateQuery({ sql: "SELECT pg_read_binary_file('pg_hba.conf', 0, 500)", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });

  it("blocks pg_ls_dir", async () => {
    await expect(validateQuery({ sql: "SELECT pg_ls_dir('.')", readOnly: true })).rejects.toMatchObject({
  ...
  it("blocks pg_file_write (adminpack)", async () => {
    await expect(
      validateQuery({ sql: "SELECT pg_file_write('/tmp/evil.sh', 'data', false)", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });

  // Server administration
  it("blocks pg_reload_conf", async () => {
    await expect(validateQuery({ sql: "SELECT pg_reload_conf()", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_FUNCTION",
    });
  });
  ...
  it("blocks pg_cancel_backend (with nested call)", async () => {
    await expect(
      validateQuery({ sql: "SELECT pg_cancel_backend(pg_backend_pid())", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  ...
  it("blocks pg_sleep", async () => {
    await expect(validateQuery({ sql: "SELECT pg_sleep(30)", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_FUNCTION",
    });
  });
  ...
  it("blocks query_to_xml", async () => {
    await expect(
      validateQuery({ sql: "SELECT query_to_xml('DELETE FROM users', true, true, '')", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  ...
  it("blocks lo_creat", async () => {
    await expect(validateQuery({ sql: "SELECT lo_creat(-1)", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_FUNCTION",
    });
  });
  ...
  it("blocks schema-qualified pg_catalog.pg_read_file", async () => {
    await expect(
      validateQuery({ sql: "SELECT pg_catalog.pg_read_file('/etc/passwd', 0, 100)", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  ...
  it("blocks dangerous function inside subquery", async () => {
    await expect(
      validateQuery({ sql: "SELECT * FROM (SELECT pg_read_file('/etc/passwd', 0, 100)) s", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  ...
  it("blocks dangerous function inside CTE", async () => {
    await expect(
      validateQuery({ sql: "WITH x AS (SELECT pg_reload_conf()) SELECT * FROM x", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
...
describe("validateQuery – expanded function denylist (readOnly=true)", () => {
  ...
  it("blocks binary_upgrade_set_next_pg_type_oid", async () => {
    await expect(
      validateQuery({ sql: "SELECT binary_upgrade_set_next_pg_type_oid(1::oid)", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  ...
  it("blocks gin_clean_pending_list", async () => {
    await expect(
      validateQuery({ sql: "SELECT gin_clean_pending_list('myidx'::regclass)", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  it("blocks brin_summarize_new_values", async () => {
    await expect(
      validateQuery({ sql: "SELECT brin_summarize_new_values('myidx'::regclass)", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  ...
  it("blocks schema_to_xml", async () => {
    await expect(
      validateQuery({ sql: "SELECT schema_to_xml('public', true, true, '')", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  it("blocks table_to_xml", async () => {
    await expect(
      validateQuery({ sql: "SELECT table_to_xml('mytable'::regclass, true, true, '')", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  ...
  it("blocks pg_stat_reset_subscription_stats", async () => {
    await expect(
      validateQuery({ sql: "SELECT pg_stat_reset_subscription_stats(1::oid)", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  ...
  it("blocks pg_backup_stop", async () => {
    await expect(validateQuery({ sql: "SELECT pg_backup_stop()", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_FUNCTION",
    });
  });
  ...
  it("blocks pg_config", async () => {
    await expect(validateQuery({ sql: "SELECT * FROM pg_config()", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_FUNCTION",
    });
  });
  ...
  it("blocks pg_replication_origin_create", async () => {
    await expect(
      validateQuery({ sql: "SELECT pg_replication_origin_create('my_origin')", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  it("blocks pg_replication_origin_drop", async () => {
    await expect(
      validateQuery({ sql: "SELECT pg_replication_origin_drop('my_origin')", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  ...
  it("blocks pg_get_backend_memory_contexts", async () => {
    await expect(
      validateQuery({ sql: "SELECT * FROM pg_get_backend_memory_contexts()", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  ...
  it("blocks pg_extension_config_dump", async () => {
    await expect(
      validateQuery({ sql: "SELECT pg_extension_config_dump('mytable', '')", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });

  // system
  it("blocks system", async () => {
    await expect(validateQuery({ sql: "SELECT system('ls')", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_FUNCTION",
    });
  });
  ...
  it("blocks pg_read_file_old", async () => {
    await expect(
      validateQuery({ sql: "SELECT pg_read_file_old('pg_hba.conf', 0, 1000)", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  ...
  it("blocks set_limit", async () => {
    await expect(validateQuery({ sql: "SELECT set_limit(0.1)", readOnly: true })).rejects.toMatchObject({
      code: "FORBIDDEN_FUNCTION",
    });
  });
  ...
  it("blocks dropgeometrycolumn", async () => {
    await expect(
      validateQuery({ sql: "SELECT dropgeometrycolumn('public', 'mytable', 'geom')", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  it("blocks dropgeometrytable", async () => {
    await expect(
      validateQuery({ sql: "SELECT dropgeometrytable('public', 'mytable')", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
  it("blocks updategeometrysrid", async () => {
    await expect(
      validateQuery({ sql: "SELECT updategeometrysrid('public', 'mytable', 'geom', 4326)", readOnly: true }),
    ).rejects.toMatchObject({ code: "FORBIDDEN_FUNCTION" });
  });
...
describe("validateQuery – allowlist skipped (readOnly=false)", () => {
  it("allows DELETE when readOnly=false", async () => {
    await expect(validateQuery({ sql: "DELETE FROM t", readOnly: false })).resolves.toBeUndefined();
  });
