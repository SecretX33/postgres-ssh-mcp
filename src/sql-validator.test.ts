...
describe('validateQuery – allowlist', () => {
  ...
  it('accepts EXPLAIN SELECT', async () => {
    await expect(
      validateQuery('EXPLAIN SELECT * FROM users'),
    ).resolves.toBeUndefined();
  });

  it('rejects INSERT', async () => {
    await expect(
      validateQuery("INSERT INTO t(x) VALUES (1)"),
    ).rejects.toMatchObject({ code: 'FORBIDDEN_STATEMENT' });
  });

  it('rejects UPDATE', async () => {
    await expect(
      validateQuery('UPDATE t SET x = 1'),
    ).rejects.toMatchObject({ code: 'FORBIDDEN_STATEMENT' });
  });

  it('rejects DELETE', async () => {
    await expect(
      validateQuery('DELETE FROM t'),
    ).rejects.toMatchObject({ code: 'FORBIDDEN_STATEMENT' });
  });

  it('rejects CREATE TABLE', async () => {
    await expect(
      validateQuery('CREATE TABLE t (id INT)'),
    ).rejects.toMatchObject({ code: 'FORBIDDEN_STATEMENT' });
  });

  it('rejects DROP TABLE', async () => {
    await expect(
      validateQuery('DROP TABLE t'),
    ).rejects.toMatchObject({ code: 'FORBIDDEN_STATEMENT' });
  });

  it('rejects TRUNCATE', async () => {
    await expect(
      validateQuery('TRUNCATE t'),
    ).rejects.toMatchObject({ code: 'FORBIDDEN_STATEMENT' });
  });
});
...
describe('validateQuery – EXPLAIN inner check', () => {
  it('rejects EXPLAIN DELETE', async () => {
    await expect(
      validateQuery('EXPLAIN DELETE FROM t'),
  ...
  it('rejects EXPLAIN UPDATE', async () => {
    await expect(
      validateQuery('EXPLAIN UPDATE t SET x = 1'),
    ).rejects.toMatchObject({ code: 'FORBIDDEN_EXPLAIN_TARGET' });
  });
});
...
describe('validateQuery – deep AST walk', () => {
  ...
  it('rejects SELECT FOR UPDATE', async () => {
    await expect(
      validateQuery('SELECT * FROM t FOR UPDATE'),
    ).rejects.toMatchObject({ code: 'FORBIDDEN_LOCKING' });
  });

  it('rejects SELECT FOR SHARE', async () => {
    await expect(
      validateQuery('SELECT * FROM t FOR SHARE'),
    ).rejects.toMatchObject({ code: 'FORBIDDEN_LOCKING' });
  });
